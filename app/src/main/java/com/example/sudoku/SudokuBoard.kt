...
class SudokuBoard(context: Context, attrs: AttributeSet?) :
    View(context, attrs) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        isValid = if (action == MotionEvent.ACTION_DOWN) {
            solver.selectedRow = ceil((y / cellSize).toDouble()).toInt()
            solver.selectedColumn = ceil((x / cellSize).toDouble()).toInt()
            true
        } else {
            false
        }
        ...
        for (r in 0..8) {
            for (c in 0..8) {
                if (solver.board[r][c] !== 0) {
                    var text: String
                    if (solver.board[r][c] < 0) {
                        text = (solver.board[r][c] * -1).toString()
                        letterPaint.color = letterColorError
                    } else {
                        letterPaint.color = letterColor
                        text = solver.board.get(r).get(c).toString()
                    }
